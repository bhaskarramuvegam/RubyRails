class NotificationBell < ActiveRecord::Base
  ...
  def self.create_for_mentions(journal)
    ...
    # (?<!\S) ensures @ is preceded by whitespace or start-of-string,
    # preventing false matches inside email addresses (user@domain.com).
    ...
    # Redmine's journals table uses created_on, not Rails' standard created_at.
    ...
    logins.each do |login|
      # Try exact login first; if not found, try treating the mention as the
      # local-part of an email login (e.g. @balaji.j → balaji.j@vegam.co).
      ...
      # Wrap in a savepoint so a DB error here does not abort the outer
      # issue-save transaction (critical for PostgreSQL).
      notification = nil
